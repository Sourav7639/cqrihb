...
export default function LoginOrganizationPage() {
  ...
  const [email, setEmail] = useState('org@demo.securyhub');
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage('Organization login successful (demo mode).');
    } catch (error) {
  ...
  return (
    <Layout>
      <Head>
        <title>Organization Login | SecuryHub</title>
      </Head>
      <section id="login-organization" className="mx-auto w-full max-w-md px-6 py-20">
        <div className="space-y-6 text-center">
          ...
          <p className="text-sm text-white/70">Sign in to create programs and triage submissions.</p>
        </div>
