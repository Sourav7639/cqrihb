...
export default function LoginAdminPage() {
  ...
  const [email, setEmail] = useState('admin@demo.securyhub');
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage('Admin login successful (demo mode).');
    } catch (error) {
  ...
  return (
    <Layout>
      <Head>
        <title>Admin Login | SecuryHub</title>
      </Head>
      <section id="login-admin" className="mx-auto w-full max-w-md px-6 py-20">
        <div className="space-y-6 text-center">
          ...
          <p className="text-sm text-white/70">View platform-level analytics and audit activity.</p>
        </div>
