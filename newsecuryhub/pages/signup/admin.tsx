...
export default function SignupAdminPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await signup({ email, name, role: 'admin', password });
      setMessage('Admin account created in demo mode.');
      setName('');
      setEmail('');
      setPassword('');
    } catch (error) {
  ...
  return (
    <Layout>
      <Head>
        <title>Admin Sign Up | SecuryHub</title>
      </Head>
      <section id="signup-admin" className="mx-auto w-full max-w-md px-6 py-20">
        <div className="space-y-6 text-center">
          ...
          <h1 className="text-3xl font-bold">Configure the platform</h1>
          <p className="text-sm text-white/70">Manage analytics, audits, and policy across SecuryHub.</p>
        </div>
