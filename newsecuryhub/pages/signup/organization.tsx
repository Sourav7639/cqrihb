...
export default function SignupOrganizationPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await signup({ email, name: company, role: 'organization', password });
      setMessage('Organization workspace created in demo mode.');
      setCompany('');
      setEmail('');
      setPassword('');
    } catch (error) {
  ...
  return (
    <Layout>
      <Head>
        <title>Organization Sign Up | SecuryHub</title>
      </Head>
      <section id="signup-organization" className="mx-auto w-full max-w-md px-6 py-20">
        <div className="space-y-6 text-center">
          ...
          <h1 className="text-3xl font-bold">Launch your program</h1>
          <p className="text-sm text-white/70">Create a workspace to publish programs and manage submissions.</p>
        </div>
