...
export default function SignupResearcherPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await signup({ email, name, role: 'researcher', password });
      setMessage('Researcher account created in demo mode.');
      setName('');
      setEmail('');
      setPassword('');
    } catch (error) {
  ...
  return (
    <Layout>
      <Head>
        <title>Researcher Sign Up | SecuryHub</title>
      </Head>
      <section id="signup-researcher" className="mx-auto w-full max-w-md px-6 py-20">
        <div className="space-y-6 text-center">
          ...
          <h1 className="text-3xl font-bold">Join the SecuryHub community</h1>
          <p className="text-sm text-white/70">Access curated programs and track your rewards.</p>
        </div>
